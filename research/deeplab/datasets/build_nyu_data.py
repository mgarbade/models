...
"""Converts nyu_depth_v2 data to TFRecord file format with Example protos.

The nyu_depth_v2 dataset is expected to have the following directory structure:

  + nyu_depth_v2
     - build_nyu_depth_v2_data.py (current working directiory).
     - build_data.py
     + nyu_depth_v2scripts
       + annotation
       + evaluation
...
nyu_depth_v2 dataset website at https://www.nyu_depth_v2-dataset.com to
download the dataset, and (2) run the script provided by nyu_depth_v2
...
tf.app.flags.DEFINE_string('nyu_depth_v2_root',
                           '/media/remote_home/mgarbade/datasets/nyu_depth_v2',
                           'nyu_depth_v2 dataset root folder.')

tf.app.flags.DEFINE_string(
    'output_dir',
    '/media/remote_home/mgarbade/datasets/nyu_depth_v2/tfrecord',
...
def _get_files(data, dataset_split):
  ...
  search_files = os.path.join(
      FLAGS.nyu_depth_v2_root, _FOLDERS_MAP[data], dataset_split, pattern)
  print(search_files)
...
def _convert_dataset(dataset_split):
  ...
  print(str(num_images) + ' files found')
